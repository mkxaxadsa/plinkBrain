...
import 'widget/levels_grid.dart';

class PairsLevelsScreen extends StatefulWidget {
  // ignore: use_super_parameters
  const PairsLevelsScreen({Key? key}) : super(key: key);

  @override
  _PairsLevelsScreenState createState() => _PairsLevelsScreenState();
}

class _PairsLevelsScreenState extends State<PairsLevelsScreen> {
  ...
  Future<List<bool>> loadCompletedLevels() async {
    ...
    List<bool> completedLevels = List.filled(9, false);

    // Загружаем сохранённые уровни
    for (int i = 0; i < completedLevels.length; i++) {
      completedLevels[i] = prefs.getBool('level_${i + 1}_completed') ?? false;
    }
...
class PairsLevelsWidget extends StatelessWidget {
  final List<bool> completedLevels;

  const PairsLevelsWidget({Key? key, required this.completedLevels})
      : super(key: key);

  Future<void> updateLevelProgress(int level) async {
    final prefs = await SharedPreferences.getInstance();
    // Mark current level as completed
    await prefs.setBool('level_${level}_completed', true);
    // Unlock next level if it exists
    if (level < 9) {
      await prefs.setBool('level_${level + 1}_completed', true);
    }
  }
  ...
  void _selectLevel(BuildContext context, int index) {
    Navigator.push(
      context,
      MaterialPageRoute(
        builder: (context) => LevelScreen(level: index),
      ),
    ).then((isLevelCompleted) {
      if (isLevelCompleted == true) {
        // Update progress if the level is completed
        updateLevelProgress(index).then((_) {
          // Refresh the screen upon returning
